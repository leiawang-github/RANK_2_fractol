...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/18 18:18:16 by leiwang           #+#    #+#             */
/*   Updated: 2025/06/24 15:02:34 by leiwang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	put_pixel_man(t_data *data, int x, int y, int color)
{
	char	*dst;
...
void	draw_mandelbrot(t_data *data)
{
	...
	while (d.x < WIDTH)
	{
		...
		while (d.y < HEIGHT)
		{
			...
			d.iter = count_man_iterations(d.z_re, d.z_im);
			d.color = get_color(d.iter, data->color_scheme);
			put_pixel_man(data, d.x, d.y, d.color);
			d.y++;
		}
		d.x++;
	}
